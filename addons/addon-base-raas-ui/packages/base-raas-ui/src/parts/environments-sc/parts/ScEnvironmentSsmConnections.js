...
class ScEnvironmentSsmConnections extends React.Component {
  constructor(props) {
    super(props);
    runInAction(() => {
      this.processingId = '';
  ...
  get connections() {
    const isSsm = scheme => scheme === 'ssm' || _.isEmpty(scheme);
    const connections = this.environment.getConnections(item => isSsm(item.scheme));
    console.log('mingtong step connections, connections: ', connections);
    return connections;
  }
  ...
  handleConnect = id =>
    action(async () => {
      ...
      console.log('mingtong step handleConnect connectInfo: ', connectInfo);
...
decorate(ScEnvironmentSsmConnections, {
  envsStore: computed,
