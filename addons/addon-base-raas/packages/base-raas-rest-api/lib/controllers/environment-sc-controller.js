...
async function configure(context) {
  ...
  // ===============================================================
  //  POST /:id/connections/:connectionId/ssm-url (mounted to /api/workspaces/service-catalog)
  // ===============================================================
  router.post(
    '/:id/connections/:connectionId/ssm-url',
    wrap(async (req, res) => {
      const id = req.params.id;
      const connectionId = req.params.connectionId;
      const requestContext = res.locals.requestContext;
      ...
      const [environmentScConnectionService] = await context.service(['environmentScConnectionService']);
      const result = await environmentScConnectionService.createConnectionSSMUrl(requestContext, id, connectionId);
      res.status(200).json(result);
    }),
  );  
