...
async function getKeycloakTokenVerifier(keycloakUrl, keycloakRealm) {
  ...
  function getSigningKey (header = decoded.header, callback) {
    console.log('getSigningKey mingtong step 0, header', header);
    keyClient.getSigningKey(header.kid, function(err, key) {
      console.log('getSigningKey mingtong step 1, key', key);
      console.log('getSigningKey mingtong step 2, key.publicKey', key.publicKey);
      console.log('getSigningKey mingtong step 3, key.rsaPublicKey', key.rsaPublicKey);
