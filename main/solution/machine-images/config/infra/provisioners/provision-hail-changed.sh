...
wget https://mingtong-public.s3.ap-northeast-1.amazonaws.com/hail/hail-on-AWS-spot-instances.zip -P /opt

cd /opt
ls
unzip hail-on-AWS-spot-instances.zip
ls
...
sudo python3 -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple $PACKAGES

sudo python3 -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple -Iv jupyterlab==2.2.6
